...
class Basic
{
    ...
    protected function setAttachmentHooks()
    {
        ...
        add_filter('wp_update_attachment_metadata', [$this, 'thumbnailUpload'], 10, 2);
        ...
        add_filter('wp_save_image_editor_file', [$this, 'handleImageEditorSave'], 10, 5);
    }
    ...
    /**
     * On Image Editor save handle reupload.
     */
    public function handleImageEditorSave($override, $filename, $image, $mime_type, $post_id)
    {
        $saved = $image->save( $filename, $mime_type );
        $local = 'public' . end(explode('public', $saved['path']));
        $remote = $this->fetchRemotePath($local);
    ...
    public function attachmentLinkRewrite($link, $post_id)
    {
        ...
        if($useCdn && $remoteSet) {
            return str_replace($this->remoteUrl, getenv('CDN_ASSET_URL'), $media->guid);
        }

        if($useCdn && !$remoteSet) {
            return getenv('CDN_ASSET_URL').$remote;
        }
    ...
    public function attachmentLinkRewriteSrcSet($sources, $size_array, $image_src, $image_meta, $attachment_id)
    {
        ...
        foreach ($sources as $key => $src) {
            ...
            if ( $useCdn ) {
                $remote = getenv('CDN_ASSET_URL');
            }
            $sources[$key]['url'] = str_replace($this->siteUrl . "/", $remote, $src['url']);
        }
